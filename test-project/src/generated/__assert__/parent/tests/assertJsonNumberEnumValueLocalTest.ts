...
export function assertJsonNumberEnumValueLocalTest( o: any, isNullable?: boolean, path : string = '__type___parent_tests_JsonNumberEnumValueLocalTest' ) {
	if (o == null) {
		if (!isNullable) {
			throw new __assert_utility___AssertionError(`Expected __type___parent_tests_JsonNumberEnumValueLocalTest, got null (at ${path})`)
		}
	} else {
		if (typeof o !== 'object') {
			throw new __assert_utility___AssertionError(
				`Expected __type___parent_tests_JsonNumberEnumValueLocalTest, got ${typeof o} (at ${path})`,
			)
		}
		__assert_utility___assertExactValue(
			...
			o.enumValue,
			false,
			path + `.enumValue:parent_tests_JsonNumberEnum:Name`
		)
		__assert_utility___assertExactValue(
			parent_tests_JsonNumberEnum.Name,
			o.enumValueWithDescription,
			false,
			path + `.enumValueWithDescription:parent_tests_JsonNumberEnum:Name`
		)
		__assert_utility___assertExactValue(
			parent_tests_JsonNumberEnum.Name,
			o.enumValueOrNull,
			true,
			path + `.enumValueOrNull:parent_tests_JsonNumberEnum:Name`
		)
